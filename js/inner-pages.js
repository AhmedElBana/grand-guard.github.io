...
//	});
    ...
    //--------------------------------- range ul width
    $('.range-body > ul').each(function(){
        var ul_size = 0 ;
        $(this).children('li').each(function(){
            ul_size++;
        });
        var colums_number = Math.ceil(ul_size / 6.0) ;
        var ul_width = colums_number * 170 ;
        $(this).css('width', ul_width );
        $(this).children('li').last().children('a').css('border','none');
        if( ul_size < 6){
        ...
        /*---- custom style ----*/
        if( $(this).hasClass('ksa-ul')){
            if( colums_number >= 3){
                $(this).parent().css({left:'auto',righ:'0px',transform:'none'});
            }
        }
        if( $(this).hasClass('katar-ul')){
            if( colums_number >= 3){
                $(this).parent().css({left:'0px',righ:'auto',transform:'none'});
            }
        }
